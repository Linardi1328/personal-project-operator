...
The connected registry powers read-only summaries, deterministic planning, issue creation, and project notes. Ordinary development runs use a separately reviewed six-project registry with fixed source paths and test policies. KHLIM Super App is eligible for one bounded Phase 1 monorepo-foundation run; its project state and test policy must be refreshed before later product-feature runs.
...
The store resolves projects only through the existing six-project registry, creates cryptographically random opaque run ids, and writes private durable records under `${PPO_WRITE_DATA_DIR}/development-runs`. Directories are `0700`; files are `0600`.
...
It accepts only a Phase 6A run in `implementation_in_progress` with a verified Phase 6C workspace. It invokes fixed non-interactive `codex exec` configuration only, keeps the controller online for OpenAI authentication, requires Codex's active no-outbound-network Linux command sandbox for generated commands, durably records bounded implementation attempts through the Phase 6A run record, sets `cwd` to the verified isolated workspace, uses a deterministic bounded prompt, creates and independently verifies one new local descendant commit after successful sandboxed edits, and then transitions the run to `implementation_ready` with metadata-only SHA-pinned implementation evidence.
...
Each invocation advances at most one outer reviewed boundary: planning, workspace preparation, Codex implementation, automated tests, independent review, bounded hardening, GitHub delivery, or SHA-pinned merge. Phase 6D, 6E, and 6F operations hold a private lease bound to the exact run, phase, attempt, implementation SHA, and owner process. A live owner blocks concurrent continuation; a dead exact owner permits one fail-closed reconciliation boundary after the orphan threshold. Reconciliation preserves verifiable Phase 6D edits, closes Phase 6E ambiguity without claiming a pass, or returns an orphaned Phase 6F attempt to its retained exact-SHA test pass. It refuses PPO self-development runs, arbitrary project/repo/SHA/action inputs, mismatched or fresh leases, unsupported statuses, and all production deployment, verification, and rollback states. The maximum routed outcome is `merged`; production continuation remains the separately approved local-only Phase 6H/6I/6J workflow.
...
It accepts only an existing opaque Phase 6A run id for one of the ordinary six projects, reads the durable run state, and dispatches at most one read-only Phase 6C-6G reconciliation or inspection boundary based on the current durable status. Phase 6G recovery remains owned by the GitHub delivery reconciler and can distinguish delivery not started, branch observed, PR observed, exact-head CI observed, remote review observed, merge-ready, and remote-merged states. Phase 6L does not retry work, mutate run state, create or remove workspaces, execute Codex, execute tests, start reviewers, push, create or modify PRs, merge, deploy, verify production, or rollback.

Phase 6L normalizes child observations into bounded metadata for owner diagnosis and re-reads the run afterward to detect concurrent state changes. Phase 6L itself remains the reviewed route-free recovery engine; Phase 6M layers the controlled route around it without changing Phase 6L policy semantics.

## Phase 6M Controlled PPO Recover Route

Phase 6M exposes the reviewed Phase 6L coordinator through the existing terminal wrapper and existing `ppo_local` direct-tool route:

```text
/ppo recover <run-id>
```

The route accepts only the opaque 43-character Phase 6A run id and invokes Phase 6L once. It is diagnostic and read-only: it does not repair state, retry work, call `/ppo continue`, create or transition runs, execute Codex, run tests, start reviewers, push, create or modify PRs, merge, deploy, verify production, rollback, or add a new OpenClaw tool or model turn. PPO self-development and production recovery remain out of scope and continue to use the separately approved local-only Phase 6H/6I/6J boundaries.

## Phase 6N - Read-Only Development Run Catalog Foundation

Phase 6N adds a local-only read-only catalog foundation:

```text
local-operator/development-run-catalog.mjs
```

It discovers only ordinary Phase 6 development runs for the existing six-project registry under the fixed `${PPO_WRITE_DATA_DIR}/development-runs` store layout. The catalog uses a separate non-mutating run-state snapshot reader instead of the existing `readDevelopmentRun(...)` recovery path, so missing stores stay missing, lagging canonical records are reported as needing recovery, and no directory creation, chmod, canonical refresh, retry, continue, cancellation, or repair is performed.

Catalog results are bounded metadata summaries: opaque run id, ordinary project id, status, stage, version, safe SHA fields, timestamps, terminal flag, and canonical-state classification. They do not expose task text, branch filesystem paths, evidence, transition history, prompts, review findings, test details, PR raw state, GitHub responses, production metadata, stdout/stderr, raw errors, environment data, credentials, tokens, or secrets. PPO self-development runs are classified out of scope and omitted from ordinary catalog listings.

Phase 6N adds no `/ppo` route, OpenClaw tool, subprocess execution, network call, recovery invocation, continuation invocation, cancellation, retry, production inspection, deployment, verification, or rollback behavior.

### Phase 6O - Controlled Read-Only Development Run Catalog Routes

Phase 6O exposes the reviewed Phase 6N catalog through the existing terminal wrapper and existing `ppo_local` direct-tool route:
...
The routes are read-only and ordinary-run only. `/ppo runs` accepts no caller filters, search text, sort fields, limits, offsets, project/status/stage selectors, paths, actions, confirmations, or production inputs. `/ppo run <run-id>` accepts exactly one opaque 43-character Phase 6A run id. Both commands reuse the Phase 6N engine, policy identity, fixed 100-record inspection bound, 20-summary output bound, active-first ordering, self-development exclusion, corrupt-content isolation, unsafe-filesystem fail-closed behavior, stale-observation fail-closed behavior, and metadata-only formatters.

The output contains only bounded catalog metadata. It does not expose task text, evidence, transition history, workspace paths, raw Git state, test output, review findings, PR/GitHub raw state, production lifecycle metadata, stdout/stderr, raw errors, environment data, credentials, tokens, or secrets. The routes never call recovery, continue, cancellation, retry, repair, deployment, production verification, rollback, a new OpenClaw tool, or a model interpretation step.

### Phase 6P - Controlled Quiescent Development Run Cancellation

Phase 6P adds confirmation-gated cancellation for existing ordinary Phase 6 development runs:
...
`/ppo cancel <run-id>` stages a single-use approval request only. It accepts exactly one opaque 43-character Phase 6A run id, inspects the run once through the reviewed Phase 6N read-only exact summary path, requires `canonical_current`, and binds the request to the exact run id, ordinary project id, current status, and expected version. Request ids are 43-character base64url values, live for 10 minutes, and are stored in the cancellation-specific private `pending-development-run-cancellations` pending/claimed store.

Cancellation is allowed only from these quiescent statuses: `created`, `planned`, `implementation_ready`, `tests_failed`, `tests_passed`, and `review_changes_requested`. In-progress statuses return `state_not_quiescent`; delivery statuses `review_passed`, `merge_ready`, and `merged` return `delivery_state_out_of_scope`; production statuses return `production_state_out_of_scope`; terminal statuses return `terminal_state`.

`/ppo cancel-confirm <request-id>` atomically claims the pending request before mutation, reinspects the run once, requires the same ordinary project, status, version, and `canonical_current` state, then performs exactly one fixed transition to `cancelled` with actor `phase-6p-quiescent-cancellation` and reason `owner_requested_quiescent_cancellation`. It does not accept caller-supplied project, status, version, SHA, branch, path, evidence, actor, reason, target status, action, cleanup option, or production input.

Phase 6P does not interrupt Codex/tests/review, remove worktrees, delete branches, close PRs, reset repositories, delete run records, invoke recovery, invoke continue, retry, repair, deploy, verify production, roll back production, add a new OpenClaw tool, or use model interpretation. PPO self-development and production runs remain out of scope.

### Phase 7A - Controlled PPO Start Route

Phase 7A exposes the reviewed Phase 6B planned-run creator through the existing terminal wrapper and existing `ppo_local` direct-tool route:
...
The route accepts exactly one caller-controlled value: one project id from the existing six-project registry (`khlim-assist`, `ledgerpilot-ai`, `spy-market-agent`, `portfolio`, `rbl-content-engine`, or `khlim-digital-ecosystem`). The Phase 7A adapter validates the exact project id, constructs only the approved internal Phase 6B invocation, invokes `createPlannedDevelopmentRun(projectId)` once, and returns bounded output with project, run id, status, next stage, base SHA, and `/ppo continue <run-id>` as the next command only after the Phase 6B planned result passes strict project/status/stage/base/head SHA consistency checks.

Telegram development is guided rather than command-memory based. Start once with `/ppo start <project>`, then copy the exact `Next command` line returned after each successful boundary. A refused or ambiguous continuation returns `/ppo run <run-id>` as the safe read-only next command instead of suggesting an unsafe retry. `/ppo menu project` lists the full start, catalog, continue, recovery, and cancellation command set.
...
Stage 0 adds a terminal-only Customer Zero controller for the fixed `Linardi1328/personal-project-operator` repository. It can start, inspect, continue, recover, and confirmation-gate cancellation for PPO self-development runs while reusing the existing Phase 6B–6G engines and stopping at `merged`. Its five local quality gates use explicit per-gate bounded timeouts, and a structurally valid open self-test attempt can be cancelled only after a 30-minute cooling period with exact-version local confirmation. A stale, structurally valid, unmerged `merge_ready` or `merge_started` state is cancellable only after the same cooling period and with a separate exact confirmation.
...
- richer Telegram/OpenClaw arbitrary text workflows beyond the approved Phase 3C text commands, Phase 5B issue approval commands, Phase 5D note approval commands, Phase 6K controlled continue command, Phase 6M read-only recover command, Phase 6O read-only run catalog commands, Phase 6P confirmation-gated quiescent cancellation commands, and Phase 7A controlled start command
- `/ppo state-promote` or any Telegram/OpenClaw project-state mutation route
- `/ppo develop`, run search, arbitrary run filters/sorts, automatic continuation after start, or background autonomous-development routing
...
- broader recovery workflows beyond the Phase 6M read-only `/ppo recover <run-id>` route
- process-interruption cancellation, retry, repair, resume, cleanup, or other run-state mutation routes beyond the Phase 6P quiescent cancellation path
