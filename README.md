...
The route accepts exactly one caller-controlled value: one project id from the existing five-project registry (`khlim-assist`, `ledgerpilot-ai`, `spy-market-agent`, `portfolio`, or `rbl-content-engine`). The Phase 7A adapter validates the exact project id, invokes `createPlannedDevelopmentRun(projectId)` once, and returns bounded output with project, run id, status, next stage, base SHA, and `/ppo continue <run-id>` as the next command when Phase 6B creates a planned run.

If Phase 6B returns `owner_action_required`, Phase 7A creates no run and returns only bounded planner outcome/reason information. It does not accept task text, repositories, paths, SHAs, branches, versions, policies, runtime options, confirmations, or actions. It does not create a workspace, invoke Codex, run tests or review, push, create a PR, merge, deploy, verify production, rollback, call `/ppo continue`, add a new OpenClaw tool, or use model interpretation. See [local-operator/phase-7a-controlled-ppo-start.md](local-operator/phase-7a-controlled-ppo-start.md) and [security/phase-7a-controlled-ppo-start.md](security/phase-7a-controlled-ppo-start.md).
