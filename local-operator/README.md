...
fields, conflicting bindings, unknown projects, and cross-workspace project use.
The validator contains no personal owner name or repository list.

`customer-zero-ownership-context.mjs` supplies the single operational binding:
owner `customer-zero-owner`, workspace `customer-zero-workspace`, and project IDs
derived from the existing approved GitHub/development registry. The registry
