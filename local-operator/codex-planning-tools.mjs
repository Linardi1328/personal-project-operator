...
function hasWriteActionSignal(domains) {
  return domains.some((domain) => domain.key === "write-actions")
}

function writeBoundaryLines(domains) {
  ...
  return [
    "",
    "Write Boundary:",
    "- Permission-gated write-action design only.",
    "- Current writes disabled in PPO.",
    "- Separate explicit approval is required before any write, mutation, merge, push, issue, PR, commit, or external write action."
  ]
}

function writeSafetyLines(domains) {
  if (!hasWriteActionSignal(domains)) {
    return []
  }
...
function chooseSplitDomains(task, estimate, domains) {
  const baseDomains = domains.length > 0
    ? domains
    : [
        {
          key: "implementation",
  ...
  if (!includeCrossCutting) {
    return baseDomains.slice(0, MAX_SPLIT_PHASES)
  }

  const writeDomain = baseDomains.find((domain) => domain.key === "write-actions")
  const hardeningPhase = baseDomains.find((domain) => domain.key === "hardening") || {
      key: "hardening",
      label: "Hardening",
  ...
  const reservedPhases = [
    ...(writeDomain ? [writeDomain] : []),
    hardeningPhase,
    reviewPhase
  ]
  const ordinaryDomains = baseDomains.filter((domain) => (
    domain.key !== "write-actions" &&
    domain.key !== "hardening" &&
    domain.key !== "review"
  ))
  const ordinarySlots = Math.max(0, MAX_SPLIT_PHASES - reservedPhases.length)

  return [
    ...ordinaryDomains.slice(0, ordinarySlots),
    ...reservedPhases
  ].slice(0, MAX_SPLIT_PHASES)
}
