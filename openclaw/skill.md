...
Phase 7A permits `/ppo start <project>` only for one allowlisted ordinary project id. It reuses Phase 6B `createPlannedDevelopmentRun(projectId)` once, creates at most one planned run, returns `/ppo continue <run-id>` as the next command, and does not automatically continue, create workspaces, invoke Codex, run tests/review, push, create PRs, merge, deploy, verify production, rollback, add a new OpenClaw tool, or use model interpretation.
