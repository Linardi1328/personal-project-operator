...
This bypasses model interpretation. The `ppo_local` tool accepts the raw `/ppo` argument string, validates it against the approved command surface, and invokes the existing wrapper with a fixed argv array. In Phase 3B, `ppo_local` keeps the Phase 2C command surface: `/ppo status`, `/ppo repo <project>`, and `/ppo pr <project>` route to GitHub read-only behavior for the four approved project ids. The local Codex prompt generator and planning tools are terminal-only and are not accepted by this bridge yet.
