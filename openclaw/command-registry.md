...
| `/ppo start` | Project Control | Create one planned ordinary development run. | `<project>` | Phase 7A controlled start | Local `gh` read-only auth | Yes, local run-state only | dangerous | Accepts only one existing ordinary-development five-project id, invokes Phase 6B `createPlannedDevelopmentRun` once, returns `/ppo continue <run-id>`, and never continues automatically, creates workspaces, invokes Codex/tests/review, pushes, creates PRs, merges, deploys, verifies production, or rolls back. |
