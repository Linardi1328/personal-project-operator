...
- Require the existing `PPO PR validation` workflow for exactly the remote PR head SHA, including successful repository syntax checks, parallel regression, full serial regression, critical Phase 6 lifecycle stability, Phase 6Q integrated user acceptance, and diff whitespace checks. Keep the required-step contract regression-bound to the checked-in workflow. Older-SHA CI, pending CI, missing CI, and failed CI must not merge.
...
- From `merge_ready`, re-fetch PR state, head SHA, base, mergeability, exact-head CI, and remote approval evidence immediately before merging. Refuse a PR reported as `behind` before recording `merge_started`; never update an approved branch in place or reuse its SHA-bound evidence on a new commit.
...
- Add exact-version, exact-confirmation cancellation for the existing quiescent status set and for a structurally valid open PPO self-test attempt only after a fixed 30-minute cooling period. Permit structurally valid stale unmerged `merge_ready` or `merge_started` evidence only after the same cooling period and with a separate exact confirmation. Do not interrupt processes, clean workspaces, delete branches, close PRs, or mutate production.
