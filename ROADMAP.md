...
- Add a separate `rollback-exact-sha.sh` primitive for the evidence-bound Phase 6J rollback. Keep `rollback-repo.sh` as the earlier manual Phase 4 recovery primitive based on `last-good-revision`.
...
- Add read-only rollback reconciliation that reports run status, expected deployment and rollback SHAs, attempt, current checkout, detached/clean marker state, runtime class, fixed service state, evidence completeness, apparent application state, retry owner-action requirement, and completion proof.
