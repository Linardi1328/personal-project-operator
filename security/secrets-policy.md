...
Phase 6F independent review uses only trusted local reviewer configuration and a deterministic bounded prompt generated from bounded task text, metadata-only Phase 6D/6E evidence, bounded local diff/file facts, and approved security/scope requirements. Review evidence may store reviewer id/hash, implementation SHA, prompt hash, decision, merge-allowed flag, bounded finding counts, bounded validated finding items, summary hash, timestamps, attempt number, aggregate outcome, sandbox id, no-network status, and read-only workspace status. It must not store prompt contents, reviewer raw stdout/stderr, raw failures, environment dumps, credentials, tokens, source repository paths, workspace absolute paths, executable paths, argv, sandbox executable paths, Linux namespace paths, read-only wrapper paths, terminal confirmation values, or unbounded logs. Review environments must be sanitized and must not inherit secret, auth, credential, confirmation, askpass, Git config, Git directory/worktree/index override, cloud, package-manager, model-provider, or home-directory env values.

Phase 6F hardening derives remediation context only from durable validated review evidence. Hardening evidence may store round number, source review SHA, resulting implementation SHA, bounded finding counts, timestamps, outcome, remediation hash, and component ids. It must not store Codex prompts, review prompts, raw model output, raw stdout/stderr, raw failures, environment dumps, credentials, tokens, arbitrary paths, executable paths, argv, terminal confirmation values, or unbounded logs.
