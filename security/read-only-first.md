...
The adapter may read one `implementation_in_progress` Phase 6A run, reconcile one verified Phase 6C workspace, establish a no-outbound-network OS/process sandbox before Codex starts, record bounded implementation attempts in the Phase 6A run record, invoke trusted locally configured Codex through that sandbox with `cwd` set to that workspace, verify a new clean local descendant commit, and transition only `implementation_in_progress -> implementation_ready`.
